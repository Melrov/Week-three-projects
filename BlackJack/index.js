  ...
  function showPlayerScore() {
    for (let i = 0; i < players.length; i++) {
      if (players[i] !== undefined) {
        ...
        playerScoreDiv[i].innerText =
          players[i].aceTotal === players[i].cardTotal
            ? `${players[i].cardTotal}`
            : players[i].aceTotal > 21
            ? `${players[i].cardTotal}`
            : `${players[i].cardTotal} / ${players[i].aceTotal}`;
  ...
  return {
    ...
    hit: () => {
      if (runningRound) {
        ...
        // console.log(players[activePlayerIndex]);
        if (
          players[activePlayerIndex].checkBust() ||
          players[activePlayerIndex].cardTotal === 21 ||
          players[activePlayerIndex].aceTotal === 21
        ) {
          updateActivePlayer();
        } else {
        }
